...
class Buku extends app\controllers\Controller {

  private $className = 'buku';
  private $classModel = 'Buku_model';
  ...
  {
    $this->data['data'] = $this->model($this->classModel)->get_data_by_request($_POST['idAjax'], 'buku', 'id');
    if($this->data['data'] > 0){
      echo json_encode($this->data['data']);
    }else{
      return false;
