...
class Member extends app\controllers\Controller {

  private $className = 'member';
  private $classModel = 'Member_model';
  ...
  {
    $this->data['data'] = $this->model($this->classModel)->get_data_by_request($_POST['idAjax'], 'member', 'id');
    if($this->data['data'] > 0){
      echo json_encode($this->data['data']);
    }else{
      return false;
